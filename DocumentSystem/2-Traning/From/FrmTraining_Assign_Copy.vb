...
Public Class FrmTraining_Assign_Copy
    ...
    Private Sub FrmTraining_Assign_Copy_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        With QEa_Training_Assign
            For Each row As DataGridViewRow In dgvDetail.Rows
                If CBool(row.Cells("Sel").Value) = True Then
                    ...
                    CmdExcute("DELETE FROM QEa_Training_Assign WHERE EmployeeNo = '" & CStr(row.Cells("EmployeeNo").Value) & "' AND Dept ='" & strDeptData & "' ", DB_CMD.ObjConn)


                    Dim Sql1 As String = "SELECT * "
                    Sql1 = Sql1 + " FROM QEa_Training_Assign "
                    Sql1 = Sql1 + " WHERE EmployeeNo = '" & FrmTraining_Assign.TxtEmployeeNo.SelectedValue.ToString & "' AND Dept ='" & strDeptData & "'"
                    ...
                    For Each Dtr1 As DataRow In Dt1.Rows
                        ...
                        sqlinsert = "INSERT INTO QEa_Training_Assign(EmployeeNo,DOCNO,DOCDEPT,MD,Dept,Assign,Trainer,MthdCode,Approve,StartDate,Remark,RetrainFreq) " &
                                        "VALUES(@EmployeeNo,@DOCNO,@DOCDEPT,@MD,@Dept,@Assign,@Trainer,@MthdCode,@Approve,@StartDate,@Remark,@RetrainFreq)"
                        ...
                        With comSave
                            ...
                            '  .Parameters.Add("@AssSeq", SqlDbType.VarChar).Value = CStr(row.Cells("AssSeq").Value)
                            .Parameters.Add("@Assign", SqlDbType.VarChar).Value = Dtr1.Item("Assign").ToString
                            .Parameters.Add("@Trainer", SqlDbType.VarChar).Value = Dtr1.Item("Trainer").ToString
                            ...
                            '    .Parameters.Add("@StartDate", SqlDbType.Date).Value = CStr(row.Cells("StartDate").Value)
                            'Else
                            '    .Parameters.Add("@StartDate", SqlDbType.Date).Value = Dtr1.Item("StartDate").ToString
                            'End If
                            ...
                            ' .Parameters.Add("@Remark", SqlDbType.VarChar).Value = Dtr1.Item("Remark").ToString
                            If IsDBNull(Dtr1.Item("RetrainFreq").ToString) Or Dtr1.Item("RetrainFreq").ToString = "" Then
                                .Parameters.Add("@RetrainFreq", SqlDbType.VarChar).Value = ""
                            Else
                                .Parameters.Add("@RetrainFreq", SqlDbType.VarChar).Value = Dtr1.Item("RetrainFreq").ToString
                            End If
